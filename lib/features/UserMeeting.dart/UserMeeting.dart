...
class _MeetingDetailsPageState extends State<MeetingDetailsPage> {
  ...
  Future<Meeting?> _getTodayMeeting() async {
    try {
      ...
      if (results["rsp"]["success"]) {
        ...
        final String message = results["rsp"]["message"] ?? "No meeting found for today.";
        showalert(
          success: false,
          context: context,
  ...
  void _showMpesaPaymentDialog(BuildContext context, {required String paymentType}) {
        ...
        return AlertDialog(
          ...
          actions: [
            TextButton(
              ...
              child: Text('Cancel', style: theme.textTheme.labelLarge?.copyWith(color: colorScheme.onSurfaceVariant)),
            ),
            ElevatedButton(
              onPressed: () {
                ...
                if (amount != null && phoneNumber.isNotEmpty) {
                  showalert(
                    ...
                    subtitle: "Request sent to $phoneNumber for KSH ${amount.toStringAsFixed(2)}. Check your phone.",
                  );
                  Navigator.of(dialogContext).pop();
                  _amountController.clear();
                  _phoneNumberController.clear();
                  ...
                  showalert(
                    ...
                    subtitle: "Please enter both amount and a valid phone number.",
                  );
                }
              },
              ...
              child: Text('Pay Now', style: theme.textTheme.labelLarge?.copyWith(color: colorScheme.onPrimary)),
            ),
          ],
        );
      },
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        leading: IconButton(
          icon: Icon(Icons.arrow_back_ios_new_rounded, color: colorScheme.onBackground),
  ...
  Widget _buildMeetingDetails(BuildContext context, Meeting meeting, ThemeData theme, ColorScheme colorScheme) {
  ...
  Widget _buildMeetingHeader(BuildContext context, Meeting meeting, ThemeData theme, ColorScheme colorScheme) {
    return Container(
        ...
        children: [
          Row(
            ...
            children: [
              Expanded(
                child: Text(
                  meeting.chamaName ?? 'Today\'s Meeting',
          ...
          Row(
            children: [
              Icon(Icons.access_time, color: colorScheme.onPrimary.withOpacity(0.9), size: 20),
              const SizedBox(width: 8),
              Text(
                '${_formatTimeOfDay(meeting.startTime)} - ${_formatTimeOfDay(meeting.endTime)}',
                style: theme.textTheme.bodyLarge?.copyWith(color: colorScheme.onPrimary.withOpacity(0.9)),
              ),
              const Spacer(),
              Icon(Icons.location_on, color: colorScheme.onPrimary.withOpacity(0.9), size: 20),
              const SizedBox(width: 8),
              Expanded(
                child: Text(
                  meeting.venue,
  ...
  Widget _buildPaymentStatusSection(BuildContext context, ThemeData theme, ColorScheme colorScheme) {
    List<Widget> pendingPayments = [];
    
    if (!_hasPaidMeetingFee) {
      pendingPayments.add(_buildPaymentCard(
        context,
        title: 'Meeting Fee',
        amount: _totalMeetingFee,
        icon: Icons.event_note,
        color: AppColors.info,
        onTap: () => _showMpesaPaymentDialog(context, paymentType: 'Meeting Fee'),
        theme: theme,
      ));
    }
    ...
    if (!_hasContributedMonthly) {
      pendingPayments.add(_buildPaymentCard(
        ...
        onTap: () => _showMpesaPaymentDialog(context, paymentType: 'Monthly Contribution'),
        theme: theme,
      ));
    }
  ...
  Widget _buildAllPaidCard(BuildContext context, ThemeData theme, ColorScheme colorScheme) {
    return Container(
  ...
  Widget _buildEssentialMeetingInfo(BuildContext context, Meeting meeting, ThemeData theme, ColorScheme colorScheme) {
    return Container(
      margin: const EdgeInsets.all(20),
      child: Card(
        elevation: 2,
          ...
          child: Column(
            ...
            children: [
              ...
              _buildInfoRow(context, Icons.calendar_today, 'Date', 
                DateFormat('EEEE, MMM d, yyyy').format(meeting.meetingDate), theme),
              const SizedBox(height: 16),
              _buildInfoRow(context, Icons.group, 'Attendance', 
                '$_presentMembersCount members present', theme),
              const SizedBox(height: 16),
              _buildInfoRow(context, Icons.person, 'Organizer', 
                meeting.createdByName!, theme),
              const SizedBox(height: 16),
              _buildInfoRow(context, Icons.account_circle, 'Your Status', 
                _userAttendanceStatus.toUpperCase(), theme),
            ],
          ),
        ),
      ),
  ...
  Widget _buildAdditionalDetails(BuildContext context, Meeting meeting, ThemeData theme, ColorScheme colorScheme) {
    return Container(
      margin: const EdgeInsets.all(20),
      child: Column(
        children: [
          ...
          Card(
            ...
            child: ExpansionTile(
              ...
              title: Text('Meeting Agenda', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
              children: [
                Padding(
                  padding: const EdgeInsets.all(20),
                  child: Text(
          ...
          Card(
            ...
            child: ListTile(
              ...
              title: Text('Previous Meeting Minutes', style: theme.textTheme.titleMedium),
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.wifi_off_rounded, color: colorScheme.onBackground.withOpacity(0.6), size: 60),
            const SizedBox(height: 16),
            Text('Connection Error', style: theme.textTheme.headlineSmall, textAlign: TextAlign.center),
            const SizedBox(height: 8),
            Text('Failed to load meeting details', style: theme.textTheme.bodyMedium, textAlign: TextAlign.center),
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.event_busy_outlined, color: colorScheme.onBackground.withOpacity(0.6), size: 60),
            const SizedBox(height: 16),
            Text('No Meeting Today', style: theme.textTheme.headlineSmall, textAlign: TextAlign.center),
            const SizedBox(height: 8),
            Text('There are no meetings scheduled for today', style: theme.textTheme.bodyMedium, textAlign: TextAlign.center),
            const SizedBox(height: 24),
            ElevatedButton.icon(
              onPressed: _fetchMeetingData,
              icon: Icon(Icons.refresh_rounded),
